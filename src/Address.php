...
class Address
{
     ...
     * @return mixed
     **/
    public static function getChinaChar($num)
    {
        $b = '';
        for ($i=0; $i<$num; $i++) {
     ...
     * @return mixed
     **/
    public static function randAddress($is_str=true)
    {
        ...
        $area = ["伊春区","带岭区","南岔区","金山屯区","西林区","美溪区","乌马河区","翠峦区","友好区","新青区","上甘岭区","五营区","红星区","汤旺河区","乌伊岭区","榆次区"];
        $road = ["爱国路","安边路","安波路","安德路","安汾路","安福路","安国路","安化路","安澜路","安龙路","安仁路","安顺路","安亭路","安图路","安业路","安义路","安远路","鞍山路","鞍山支路","澳门路","八一路","巴林路","白城路","白城南路","白渡路","白渡桥","白兰路","白水路","白玉路","百安路（方泰镇）","百官街","百花街","百色路","板泉路","半淞园路","包头路","包头南路","宝安公路","宝安路","宝昌路","宝联路","宝林路","宝祁路","宝山路","宝通路","宝杨路","宝源路","保德路","保定路","保屯路","保屯路","北艾路"];
        $home = ["金色家园","耀江花园","阳光翠竹苑","东新大厦","溢盈河畔别墅","真新六街坊","和亭佳苑","协通公寓","博泰新苑","菊园五街坊","住友嘉馨名园","复华城市花园","爱里舍花园"];
      
        $province=$province[rand(0, count($province)-1)];
        $city=$city[rand(0, count($city)-1)];
        $area=$area[rand(0, count($area)-1)];
        $road=$road[rand(0, count($road)-1)];
        $home=$home[rand(0, count($home)-1)];
        $num=rand(0, 200).'号';
        if ($is_str) {
            $result=sprintf('%s%s%s%s%s%s', $province, $city, $area, $road, $num, $home);
        } else {
            $result=[$province,$city,$area,$road,$num,$home];
        }
        return $result;
    }
}
